...
`.wikiignore` at the **repository root** (not inside `wiki/`) lists paths the wiki must never read when surveying source, verifying pages, or drafting updates. It complements `.gitignore`: agents must treat **both** as off limits.
