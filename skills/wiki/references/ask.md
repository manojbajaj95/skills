...
4. **Verify only if needed.** If the wiki looks stale, contradicts the question, or lacks coverage, say so, confirm against source (respecting `.wikiignore` and `.gitignore` — [wikiignore.md](wikiignore.md)), then answer — and suggest **update** if the wiki is out of date.
